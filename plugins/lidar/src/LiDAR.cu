...
void LiDARcloud::calculateLeafAreaGPU_synthetic( helios::Context* context, bool beamoutput, bool fillAnalytic  ){
    ...
    for( uint s=0; s<Nscans; s++ ){
       ...
       //         std::vector<helios::vec3> xyz_filled = gapfillMisses(s);
    //    this_scan_xyz.insert( this_scan_xyz.end(), xyz_filled.begin(), xyz_filled.end() );
     //   std::cout << "this_scan_xyz.size() = " << this_scan_xyz.size() << std::endl; 
        ...
        for(uint i=0; i < Nhits; i++)
        {
            ...
            {
                beam_array.at(beam_ID).push_back(i);
            }else{
                beam_ID ++;
                beam_array.at(beam_ID).push_back(i);
        ...
        //ERK
            if( printmessages ){
            std::cout << "Created beam_array vector" << std::endl;
        }
...
//ERK
void LiDARcloud::calculateLeafAreaGPU_equal_weighting( helios::Context* context, bool beamoutput, bool fillAnalytic  ){
    ...
    if( printmessages ){
        std::cout << "finished dr calculations" << std::endl;
    }
    ...
    std::ofstream file_output;
    file_output.open("../voxeloutput/voxeloutput.txt");
    file_output << "cell, grid_center_x, grid_center_y, grid_center_z, grid_size_x, grid_size_y, grid_size_z, Nbeams, dr_bar, dr_var, G, P_equal, LAD_equal"  << std::endl;
    ...
    for(uint c=0; c<Ncells; c++)
    {
        ...
        file_output << c << "," << grid_center.x << "," << grid_center.y << "," << grid_center.z << "," << grid_size.x << "," << grid_size.y << "," << grid_size.z << "," << voxel_beam_count_tot.at(c) << 
            "," << dr_bar_ref[c] << "," << dr_var_ref[c] << "," << Gtheta[c] << "," << P_equal[c] << "," << LAD_equal[c] << std::endl;
    }
    file_output.close();
