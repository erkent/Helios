...
void LiDARcloud::syntheticScan_UUID_grouping( helios::Context* context, int rays_per_pulse, float pulse_distance_threshold, bool scan_grid_only, bool record_misses ){
  ...
  for( int s=0; s< getScanCount(); s++ ){
    ...
    for( size_t r=0; r<N; r++ ){
      ...
      if( t_pulse.size()==1 ){ //this is discrete-return data, or we only had one hit for this pulse
        ...
        //ERK 
        // UUID / ObjID method
        std::vector<uint> current_beam_UUIDs;
        for( size_t hit=0; hit<t_pulse.size(); hit++ ){
          current_beam_UUIDs.push_back( t_pulse.at(hit).at(2));
        }
        sort(current_beam_UUIDs.begin(), current_beam_UUIDs.end());
        std::vector<uint>::iterator it;
        it = std::unique(current_beam_UUIDs.begin(),current_beam_UUIDs.end());
        current_beam_UUIDs.resize( std::distance(current_beam_UUIDs.begin(),it) );
        
        for(uint cbu=0;cbu<current_beam_UUIDs.size();cbu++)
        {
          std::cout << "current_beam_UUIDs = " << current_beam_UUIDs.at(cbu) << std::endl;
          
        }
  
        // std::vector<uint> current_beam_objectIDs = context->getUniquePrimitiveParentObjectIDs(current_beam_UUIDs);
        
        // for(uint cbu=0;cbu<current_beam_objectIDs.size();cbu++)
        // {
        //   std::cout << "current_beam_objectIDs = " << current_beam_objectIDs.at(cbu) << std::endl;
        //   
        // }
        
        std::vector<uint> groupingIDs = current_beam_UUIDs;
        // if (current_beam_objectIDs.empty()){
        //   groupingIDs = current_beam_UUIDs;
        // }else{
        //   groupingIDs = current_beam_objectIDs;
        // }
        
        // for(uint cbu=0;cbu<groupingIDs.size();cbu++)
        // {
        //   std::cout << "groupingIDs = " << groupingIDs.at(cbu) << std::endl;
        //   
        // }
        
        // each unique UUID represented in the rays
        for(uint cr=0;cr<groupingIDs.size();cr++)
        {
          uint current_group_ID = groupingIDs.at(cr);
          // std::cout << "current_group_ID = " << current_group_ID << std::endl;
          // float t0=t_pulse.at(0).at(0);
          // float d=t_pulse.at(0).at(0);
          // float f=t_pulse.at(0).at(1);
          float d=0.0;
          ...
          for( size_t hit=0; hit<t_pulse.size(); hit++ )
          {
            // uint current_ray_group_ID  = context->getPrimitiveParentObjectID(t_pulse.at(hit).at(2)); 
            uint current_ray_group_ID  = t_pulse.at(hit).at(2); 
            // std::cout << "current_ray_group_ID = " << current_ray_group_ID << std::endl;
            // the ray hit the current ObjID/UUID
            if(current_group_ID == current_ray_group_ID)
            {
              count++;
              d+=t_pulse.at(hit).at(0);
              f+=t_pulse.at(hit).at(1);
...
void LiDARcloud::syntheticScan_delta_distance_grouping( helios::Context* context, int rays_per_pulse, float pulse_distance_threshold, bool scan_grid_only, bool record_misses ){
  ...
  for( int s=0; s< getScanCount(); s++ ){
    ...
    for( size_t r=0; r<N; r++ ){
      ...
      if( t_pulse.size()==1 ){ //this is discrete-return data, or we only had one hit for this pulse
        ...
        std::sort( t_pulse.begin(), t_pulse.end(), LIDAR_CUDA::sortcol0 );
        std::vector<float> diff_distance;
        diff_distance.resize(t_pulse.size() - 1);
        //loop over rays and calculate normalized differences in distance
        for( size_t hit=0; hit<(t_pulse.size()-1); hit++ ){
          diff_distance.at(hit) = (t_pulse.at(hit + 1).at(0) - t_pulse.at(hit).at(0))/t_pulse.at(hit).at(0); 
        }
        // find peaks in the normalized difference in distance
        // these are the indices of t_pulse
        std::vector<uint> peaks = peakFinder(diff_distance);
        
        // now get the values
        std::vector<float> peak_values;
        // std::cout << "peaks detected at :" << std::endl;
        for(uint jj=0;jj <peaks.size();jj++)
        {
        ...
        for( size_t hit=1; hit<=t_pulse.size(); hit++ ){
          ...
          // std::cout << "t_pulse.at(hit).at(0) = " << t_pulse.at(hit).at(0) << ", t_pulse.at(peak_values.at(peakIndex)).at(0) = " << peak_values.at(peakIndex) << std::endl;
          // 
          // 
          // if the end has been reached, output the last hitpoint
          if( hit == t_pulse.size()){
            ...
            // else if the current ray is in the next distance bin  it is part of the next hitpoint so output the previous hitpoint and reset
          }else if( t_pulse.at(hit).at(0) > peak_values.at(peakIndex) ){
            ...
            // or else the current ray is in the current bin and is part of current hitpoint; add it to the current hit point and continue on
          }else{
...
void LiDARcloud::syntheticScan_delta_distance_Tpd_grouping( helios::Context* context, int rays_per_pulse, float pulse_distance_threshold, bool scan_grid_only, bool record_misses ){
  ...
  for( int s=0; s< getScanCount(); s++ ){
    ...
    for( size_t r=0; r<N; r++ ){
      ...
      std::vector<std::vector<float> > t_pulse;
      std::vector<std::vector<float> > t_hit;
      std::vector<std::vector<float> > t_hit_temp;
      ...
      float d0 = t_hit_temp.at(0).at(0);
      float f0 = t_hit_temp.at(0).at(1);
      float nr = float(t_hit_temp.at(0).at(2));
      
      //loop over hit points
      for( size_t hit=1; hit<=t_hit_temp.size(); hit++ ){
        
        // if the end has been reached, output the last hitpoint
        if( hit == t_hit_temp.size()){
          
          float distance = d0;
          float intensity = f0;
          float nPulseHit = float(nr);
          float IDmap = t_hit_temp.at(hit-1).at(2);
        ...
        }else if( t_hit_temp.at(hit).at(0)-d0 > pulse_distance_threshold ){
          ...
          d0 = t_hit_temp.at(hit).at(0);
          f0 = t_hit_temp.at(hit).at(1);
          nr = float(t_hit_temp.at(hit).at(2));
          ...
          d0 = (d0*f0 + t_hit_temp.at(hit).at(0)*t_hit_temp.at(hit).at(1))/(f0 + t_hit_temp.at(hit).at(1));
          f0 = f0 + t_hit_temp.at(hit).at(1);
          nr = nr + float(t_hit_temp.at(hit).at(2));
        }
...
void LiDARcloud::syntheticScan_Tpd_alt( helios::Context* context, int rays_per_pulse, float pulse_distance_threshold, bool scan_grid_only, bool record_misses ){
